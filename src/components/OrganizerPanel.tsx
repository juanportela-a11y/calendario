...
export const OrganizerPanel: React.FC<OrganizerPanelProps> = ({
  ...
  const canManageEvents = currentUser?.rol === 'organizador' || currentUser?.rol === 'administrador';

  if (!canManageEvents) {
    return (
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 text-center max-w-xl mx-auto space-y-4 shadow-sm my-8">
        <div className="w-16 h-16 bg-amber-100 dark:bg-amber-950/70 text-amber-600 dark:text-amber-400 rounded-3xl flex items-center justify-center mx-auto shadow-inner">
          <Building2 className="w-8 h-8" />
        </div>
        <h3 className="text-xl font-black text-slate-900 dark:text-white">
          Acceso Restringido a Organizadores y Administradores
        </h3>
        <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
          Solo las entidades organizadoras autorizadas (Alcaldía, Casa de la Cultura, Parroquia, IDER, EMPUR) y la cuenta de Administración Municipal tienen permisos para crear y modificar eventos en el calendario oficial de Purificación.
        </p>
        <div className="p-3.5 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700 text-xs text-slate-700 dark:text-slate-300">
          Tu rol actual es: <strong className="uppercase text-blue-600 dark:text-blue-400">{currentUser?.rol || 'Invitado'}</strong> ({currentUser?.nombre_usuario || 'Sin sesión'}).
        </div>
      </div>
    );
  }
