      ...
      {activeScreenTab === 'formulario' && (
        <div className="bg-white dark:bg-slate-900 rounded-[36px] shadow-sm border border-slate-200/80 dark:border-slate-800 p-6 sm:p-10 md:p-12 transition-all animate-fade-in">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 max-w-xl mx-auto">
            ...
            {/* BOTÓN Y ACCIÓN: AÑADIR UBICACIÓN ACTUAL */}
            <div className="flex flex-col items-center justify-center gap-2 pt-1">
              <button
                type="button"
                onClick={handleGetLocation}
                disabled={isLocating}
                className="inline-flex items-center gap-1.5 text-xs sm:text-sm font-bold text-[#2563EB] dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 hover:underline cursor-pointer transition-colors"
              >
                <Navigation className={`w-3.5 h-3.5 ${isLocating ? 'animate-spin' : ''}`} />
                <span>{isLocating ? 'Detectando GPS...' : (locationLabel ? `📍 ${locationLabel}` : 'Añadir ubicación actual')}</span>
              </button>

              {/* Toggle / Quick Barrio & Direccion Picker */}
              <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 rounded-xl bg-slate-100/90 dark:bg-slate-800 text-xs font-semibold text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  >
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="Ej: Cra 7 # 4-20 frente a tienda"
                    value={direccionExacta}
                    onChange={(e) => setDireccionExacta(e.target.value)}
                    className="w-full px-3 py-2 rounded-xl bg-slate-100/90 dark:bg-slate-800 text-xs font-medium text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-blue-500 focus:outline-none placeholder:text-slate-400"
                  />
                </div>
              </div>
            </div>
